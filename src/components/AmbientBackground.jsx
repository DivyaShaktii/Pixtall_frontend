 ...
 * Features:
 * - Fine grid texture (very low opacity)
 * - Slow-drifting, heavily blurred lime glow
 * - A faint rounded accent shape anchored in a corner
 * - prefers-reduced-motion: crossfades to static (no animation, elements still render)
 * 
 * @param {"dark" | "light"} variant - "dark" for marketing/auth pages, "light" for post-login Studio
 */
export default function AmbientBackground({ variant = "dark" }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none select-none" aria-hidden="true">
      ...
      {/* Corner accent shape — a faint rounded rectangle anchored bottom-right */}
      <div 
        className={`absolute -bottom-20 -right-20 w-64 h-64 ${accentColor} rounded-[3rem] rotate-12 blur-[2px]`} 
      />
    </div>
  );
}
